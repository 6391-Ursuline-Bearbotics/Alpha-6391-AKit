...
public class WindupXboxController extends CommandXboxController {
    ...
    /**
     * Return a Command that rumbled both sides of the driver controller assumes full intensity
     * since that parameter is not provided.
     */
    public Command rumbleForTime(double seconds)
    {
        return Commands.startEnd(() -> {
            timer.restart();
            m_driveRmbl.setRumble(GenericHID.RumbleType.kBothRumble, 1.0);
        },
            () -> {
                m_driveRmbl.setRumble(GenericHID.RumbleType.kBothRumble, 0);
            })
